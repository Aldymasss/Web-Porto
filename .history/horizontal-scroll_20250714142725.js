...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nextBtn) {
      ...
      // Start auto-scrolling after a short delay to ensure content is fully loaded
    setTimeout(startAutoScroll, 1500);
    
    // Handle window resize to reset scroll position
    window.addEventListener('resize', () => {
        // Pause scrolling during resize
        isPaused = true;
        
        // Resume scrolling after resize is complete
    ...
    // Ensure scrolling works correctly when tab regains focus
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            // Tab is now visible, reset scrolling
            startAutoScroll();
        } else {
            // Tab is hidden, pause scrolling
            isPaused = true;
        }
    });
});
